...
public class FileToObjectRDDService implements Serializable, IndexParamInterface, GeoInterface {
					...
					if(line.length() > 3){
						...
						for(int i=1; i<samplePoints.length; i++){
							...
							if(attributes.length > 4){
								...
								pointList.add(new Point(lon, lat, time));
							}
						}
					}
	...
	/**
	 * Convert a input dataset file RDD to trajectory objects.
	 * Use Spark MapReduce.
	 * 
			...
			public Iterable<Trajectory> call(Iterator<String> partition) throws Exception {
				...
				while(partition.hasNext()){
					...
					if(line.length() > 3){
					    ...
					    trajectory.addPoint(new Point(lon_ini, lat_ini, time_ini));
						...
						for(int i=1; i<samplePoints.length; i++){
							...
							if(attributes.length > 4){
								...
								// new point for this input
								trajectory.addPoint(new Point(lon, lat, time));
							}
						}
	 ...
	 * Save to HDFS if required, save as 'sample-points-merc'.
	...
	public List<Trajectory> selectSampleTrajectories(
     	...
     	// map the input dataset to point objects
 	 	JavaRDD<Trajectory> pointsRDD = mapRawDataToTrajectoryRDD(fileRDD);
