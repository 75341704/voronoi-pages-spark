 ...
 * 
 * @author uqdalves, Zhenyu Pan
 *
 */
@SuppressWarnings("serial")
public class VoronoiDiagramGenerator implements Serializable, GeoInterface {
     ...
     * </br>
     * Uses Fortune's algorithm for Voronoi edges calculation.
     */
    public List<VoronoiPolygon> generateVoronoiPolygons(List<Point> pointsList){
    	// initially creates an empty polygon for each point in the list
    	List<VoronoiPolygon> polyList = 
    			new ArrayList<VoronoiPolygon>();
    	for(Point pivot : pointsList){
    		VoronoiPolygon vp = 
    				new VoronoiPolygon(pivot);
    		polyList.add(vp);
    	}
    	
    	// generate the polygons edges
    	List<VoronoiEdge> edges = 
    			generateVoronoiEdges(pointsList);
     ...
     */
    public List<VoronoiEdge> generateVoronoiEdges(List<Point> pointsList){
		...
		return generateVoronoi(xValuesIn, yValuesIn, MIN_X, MAX_X, MIN_Y, MAX_Y);
    }
     ...
     */
    private List<VoronoiEdge> generateVoronoi(double[] xValuesIn, double[] yValuesIn,
    ...
    private Segment bisect(Site s1, Site s2)
    {
    ...
    private HalfEdge HEcreate(Segment e, int pm)
    {
        HalfEdge answer;
        answer = new HalfEdge();
        answer.ELedge = e;
    ...
    private void pushGraphEdge(Site leftSite, Site rightSite, double x1, double y1, double x2, double y2)
    {
        ...
        newEdge.pivot1 = leftSite.sitenbr;
        newEdge.pivot2 = rightSite.sitenbr;
    }

    private void clip_line(Segment e)
    {
    ...
    private void endpoint(Segment e, int lr, Site s)
    {
        e.ep[lr] = s;
        if (e.ep[RE - lr] == null)
        {
