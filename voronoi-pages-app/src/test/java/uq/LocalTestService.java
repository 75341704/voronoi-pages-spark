...
/*		
	...
	/*		
		...
		set1.add(1);set1.add(2);set1.add(3);set1.add(4);set1.add(5);set1.add(1);
		HashSet<Integer> set2 = new HashSet<Integer>();
		set2.add(1);set2.add(2);set2.add(6);set2.add(7);set2.add(8);
		
		System.out.println("Set1: "+set1.toString());
		System.out.println("Set2: "+set2.toString());
		
		set2.removeAll(set1);
		System.out.println("Set2-1: "+set2.toString());
		set1.addAll(set2);
		System.out.println("Set1+2: "+set1.toString());
		*/
		/*
		computeAccuracy();
		*/
	...
	public static void computeAccuracy(){
		// file to read
		File fileGrid = new File("C:/lol/grid");
		File fileVoro = new File("C:/lol/voronoi");
		try {
			BufferedReader bufferGrid = new BufferedReader(
					new FileReader(fileGrid));
			BufferedReader bufferVoro = new BufferedReader(
					new FileReader(fileVoro));
			
			// each line of the current file
			String line;
			String script_prec = "VPages(E-4)\t\t\tGPages(E-4)\n";
			String script_num = "VPages\t\t\tGPages\n";
			double prec_v, prec_g;
			long num_v, num_g;
			while(bufferGrid.ready()) {
				// read grid lines
				line = bufferGrid.readLine();
				line = bufferGrid.readLine().substring(4);
				num_g = Long.parseLong(line) + 20;
				line = bufferGrid.readLine();
				line = bufferGrid.readLine();
				line = bufferGrid.readLine();
				line = bufferGrid.readLine().substring(11);
				prec_g = Double.parseDouble(line) * 10000;
				line = bufferGrid.readLine();
				line = bufferGrid.readLine();
				
				// read voronoi file lines					
				line = bufferVoro.readLine();
				line = bufferVoro.readLine().substring(4);
				num_v = Long.parseLong(line) + 20;
				line = bufferVoro.readLine();
				line = bufferVoro.readLine();
				line = bufferVoro.readLine();
				line = bufferVoro.readLine().substring(11);
				prec_v = Double.parseDouble(line) * 10000;
				line = bufferVoro.readLine();
				line = bufferVoro.readLine();
				
				script_prec += prec_v + "\t\t\t" + prec_g + "\n";
				script_num  += num_v + "\t\t\t" + num_g + "\n";
			}
			System.out.println(script_prec);
			System.out.println();
			System.out.println();
			System.out.println(script_num);
	...
	public static void convertUseCasesToMercator(){
		...
		try {
			...
			BufferedWriter writer = new BufferedWriter(
					new FileWriter(new File("C:/lol/spatial-temporal-mercator") ));
			writer.write(script);
			writer.flush();
		...
		try {
			...
			while(buffer.ready()){
				...
				for(int i=1; i<tokens.length-2; i+=3){
					...
					long time = Long.parseLong(tokens[i+2]);
				...
				// only add trajectories with 100 to 1.000 points
				if(t.size() >= 100 && t.size() <= 1000){
					script += t.toString() + "\n";
				}
			...
			script = script.substring(0, script.length()-1);
			BufferedWriter writer = new BufferedWriter(
					new FileWriter(new File("C:/lol/nn-mercator") ));
			writer.write(script);
			writer.flush();
	...
	public static void compareResults(){
		// file to read
		File fileGreedy = new File("C:/lol/greedy");
		File fileVoronoi = new File("C:/lol/voronoi");
		try {
			BufferedReader bufferGreedy = new BufferedReader(
					new FileReader(fileGreedy));
			BufferedReader bufferVoronoi = new BufferedReader(
					new FileReader(fileVoronoi));
			...
			for(int i=0; i<100; i++){
				// skip first line
				lineVoronoi = bufferVoronoi.readLine();
				lineGreedy = bufferGreedy.readLine();
				// read query results (10-NN)
				for(int j=0; j<10; j++){
					// read voronoi
					lineVoronoi = bufferVoronoi.readLine();
					String[] tokensVoronoi = lineVoronoi.replace("[", " ").split(" ");
					
					// read greedy
					lineGreedy = bufferGreedy.readLine();
					String[] tokensGreedy = lineGreedy.replace("[", " ").split(" ");
					...
					if(tokensVoronoi[1].equals(tokensGreedy[1])){
						...
						System.out.println("Voronoi: " + lineVoronoi);
						System.out.println("Greedy: " + lineGreedy);
						System.out.println();
					}
				}
				// read the next 90 lines of the greedy
				for(int j=0; j<90; j++){
					lineGreedy = bufferGreedy.readLine();
				}
